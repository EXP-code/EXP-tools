...
def lm_index(ele, m):
    """
    Calculate the index of a spherical harmonic element in the pyEXP given the angular numbers l and m .
    ...
    """
    assert isinstance(ele, int) and isinstance(m, int), "l and m must be integers"
    assert ele >= 0, "l must be greater than 0"
    assert abs(m) <= ele, "m must be less than or equal to l"
    return int(ele * (ele + 1) / 2) + abs(m)

def nlm_index(n, ele, m, lmax):
    """
    ...
    """
    assert isinstance(n, int) and isinstance(ele, int) and isinstance(m, int), "all args must be integers"
    ...
    return lm_index(ele, m) + n_idx

def inverse_index_lm(i_lm):
    """
    Calculate the angular numbers l and m given the index of a spherical harmonic element.

    Parameters:
        index_lm (int): The index of the spherical harmonic element.

    Returns:
        tuple: A tuple containing the angular numbers (l, m).
    """
    import math
    assert isinstance(i_lm, int) and i_lm >=0, "I must be an interger greater than or equal to 0"
    ele = math.floor((-1 + math.sqrt(1 + 8 * i_lm)) / 2)  # Calculate l using the inverse of the formula
    m = i_lm - int(ele * (ele + 1) / 2)  # Calculate m using the given formula
    return ele, m

def inverse_index_nlm(i_lmn, lmax):
    """
    ...
    """
    assert isinstance(i_lmn, int) and i_lmn >=0, "I must be an interger greater than or equal to 0"
    terms_per_n = int(total_terms(l_max=lmax))
    n = i_lmn//terms_per_n
    Irem = i_lmn%terms_per_n
    ele,m = inverse_index_lm(Irem)
    return n, ele, m
...
def set_specific_lm_non_zero(data, lm_pairs_to_set):
    ...
    for ele, m in lm_pairs_to_set:
        if ele < 0 or ele >= data.shape[0] or m < 0 or m > ele:
            raise ValueError(f"Invalid (l, m) pair: ({ele}, {m}). Out of bounds for the input data shape.")
        arr_filt[lm_index(ele, m), :, :] = data[lm_index(ele, m), :, :] 
...
def listStates(n, lmax, allStates=True):
    ...
    list
        ...
        as (n,l,m)
    """
    if allStates is False:
        nmin, nmax = n, n+1
    else:
        nmin, nmax = 0, n+1
    states = [(n,ele,m) for n in range(nmin, nmax) 
                       for ele in range(0, lmax +  1) 
                         for m in range(0, lmax + 1) if m<=ele]
    return states
